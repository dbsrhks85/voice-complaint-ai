...
def resolve_report(report_id: int, new_status: str = Form(default="completed")):
    """
    민원 처리 상태 변경 (관리자용)
    new_status: pending | processing | completed
    """
    ...
    result = supabase.table("complaints").update(update_data).eq("id", report_id).execute()

    if not result.data:
        raise HTTPException(status_code=404, detail="해당 ID의 민원을 찾을 수 없습니다.")
